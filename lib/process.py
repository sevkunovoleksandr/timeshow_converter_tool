...
def safe_time(value):
    """
    Конвертує значення у float, навіть якщо воно має дивний формат типу '4.4.00'.
    Повертає 0.0, якщо не вдалось розпарсити.
    """
    ...
    # Якщо формат типу '4.4.00' → беремо тільки перші дві частини
    if s.count('.') > 1:
        parts = s.split('.')
        s = '.'.join(parts[:2])
...
def process(s: str) -> dict[str, str]:
    ...
    duration = project.get("length") or 0
    try:
        duration = float(duration)
    except ValueError:
        duration = 0.0

    tc = CreateTimecode(root_tc, name=project["export_lable"], duration=duration)
    ...
    for stack in data["stacks"]:
        ...
        for action in stack["actions"]:
            ts_val = safe_time(action.get("time_stamp", 0))
            evt = CreateCmdEvent(
                sub,
            ...
            CreateRealtimeCmd(
                evt,
                obj=targ,
                val_cue_dest=targRel + "." + action["actions_lable"]
            )

    # ~~ Sequences ~~
    root_seq = CreateGma3Root()
    for stack in data["stacks"]:
        col = stack.get("stack_color", "")
        appearance = ""
        rgb = None
        if col:
            rgb = hex_to_rgb(col)
        ...
        for i, action in enumerate(stack["actions"], start=1):
            cue = CreateCue(seq, name=action["actions_lable"], no=action["action_id"], allow_dupes="")
            CreatePart(
                cue,
                name=action["actions_lable"],
                cue_in_fade=action["action_value"],
                sync="",
                morph=""
            )
